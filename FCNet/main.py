...
from builtin import NetBuiltin
import sys
from manual import NetManual

modules = sys.modules

manual_lines = open('manual.py', 'r').readlines()
invalid_strings = ['import torch', 'from torch', 'importlib', 'torch.']
invalid_lines = [line for line in manual_lines if any(k for k in invalid_strings
                                                      if k in manual_lines)]
if invalid_lines:
    invalid_lines_str = ''.join(invalid_lines)
    raise AssertionError(
        f'invalid_strings found in manual_lines:\n{invalid_lines_str}')
