...
from submission_builtin import NetBuiltin
import sys

from submission_manual import NetManual

modules = sys.modules

submission_manual_lines = open('submission_manual.py', 'r').readlines()
invalid_strings = ['import torch', 'from torch', 'importlib', 'torch.']
invalid_lines = [line for line in submission_manual_lines if any(k for k in invalid_strings
                                                                 if k in submission_manual_lines)]
if invalid_lines:
    invalid_lines_str = ''.join(invalid_lines)
    raise AssertionError(
        f'invalid_strings found in submission_manual_lines:\n{invalid_lines_str}')
...
def get_dataloaders(batch_size):
    ...
    MNIST_train_set, MNIST_val_set = random_split(
        MNIST_training, [55000, 5000])

    train_loader = torch.utils.data.DataLoader(
        MNIST_train_set, batch_size=batch_size.train, shuffle=True)

    val_loader = torch.utils.data.DataLoader(
        MNIST_val_set, batch_size=batch_size.val, shuffle=True)
